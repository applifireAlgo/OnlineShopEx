Ext.define('Onlineshopping.onlineshopping.web.shop.controller.retail.CardMainController', {
     extend: 'Onlineshopping.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CardMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CardTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#CardGrid');
               grid.store.add(data);
               var tree = this.view.down('#CardTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Card');
          var grid = this.view.down('#CardGrid');
          var tree = this.view.down('#CardTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CardGrid');
               ...
               var formPanel = this.getView().up().down("#Card");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#CardTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Card').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CardGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.cardId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Card').getViewModel().setData(responseData.response.data);
     },
