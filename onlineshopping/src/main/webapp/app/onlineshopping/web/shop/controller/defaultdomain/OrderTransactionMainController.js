Ext.define('Onlineshopping.onlineshopping.web.shop.controller.defaultdomain.OrderTransactionMainController', {
     extend: 'Onlineshopping.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.OrderTransactionMainController',
     ...
     orderIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/OrderMain/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#orderId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'orderId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#OrderTransactionTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#OrderTransactionGrid');
               grid.store.add(data);
               var tree = this.view.down('#OrderTransactionTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#OrderTransaction');
          var grid = this.view.down('#OrderTransactionGrid');
          var tree = this.view.down('#OrderTransactionTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#OrderTransactionGrid');
               ...
               var formPanel = this.getView().up().down("#OrderTransaction");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#OrderTransactionTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#OrderTransaction').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#OrderTransactionGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.mapId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#OrderTransaction').getViewModel().setData(responseData.response.data);
     },
